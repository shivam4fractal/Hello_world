with enriched as (
  select * from {{ ref(int_inet_loads_enriched) }}
)
select
  -- internal_order_id_nk
  CAST(enriched.internal_order_id_nk AS BIGINT) as internal_order_id_nk,
  -- internal_load_id_nk
  CAST(enriched.internal_load_id_nk AS BIGINT) as internal_load_id_nk,
  -- costid_nk
  CAST(enriched.costid_nk AS DECIMAL(18,2)) as costid_nk,
  -- internal_orderdetail_id_nk
  CAST(enriched.internal_orderdetail_id_nk AS BIGINT) as internal_orderdetail_id_nk,
  -- to_bonus_time
  enriched.to_bonus_time as to_bonus_time,
  -- to_order_number
  enriched.to_order_number as to_order_number,
  -- to_load_number
  enriched.to_load_number as to_load_number,
  -- ld_load_name
  UPPER(TRIM(enriched.ld_load_name)) as ld_load_name,
  -- means_of_transport
  enriched.means_of_transport as means_of_transport,
  -- to_special_order_type
  enriched.to_special_order_type as to_special_order_type,
  -- to_act_pickup_date
  CAST(enriched.to_act_pickup_date AS DATE) as to_act_pickup_date,
  -- co_total_costs
  CAST(enriched.co_total_costs AS DECIMAL(18,2)) as co_total_costs,
  -- co_cost_type
  CAST(enriched.co_cost_type AS DECIMAL(18,2)) as co_cost_type,
  -- to_weight_unit
  CAST(enriched.to_weight_unit AS DECIMAL(18,4)) as to_weight_unit,
  -- to_gross_weight
  CAST(enriched.to_gross_weight AS DECIMAL(18,4)) as to_gross_weight,
  -- packagetype
  enriched.packagetype as packagetype,
  -- to_consigner_city
  UPPER(TRIM(enriched.to_consigner_city)) as to_consigner_city,
  -- to_consigner_country
  UPPER(TRIM(enriched.to_consigner_country)) as to_consigner_country,
  -- lane_name
  UPPER(TRIM(enriched.lane_name)) as lane_name,
  -- route_distance
  CAST(enriched.route_distance AS DECIMAL(18,4)) as route_distance,
  -- to_purchase_order_number
  enriched.to_purchase_order_number as to_purchase_order_number,
  -- to_consignor_id
  CAST(enriched.to_consignor_id AS BIGINT) as to_consignor_id,
  -- totalpalletsactual
  enriched.totalpalletsactual as totalpalletsactual,
  -- totalpalletsplaned
  enriched.totalpalletsplaned as totalpalletsplaned,
  -- to_status
  UPPER(TRIM(enriched.to_status)) as to_status,
  -- to_floor_pallets_actuals
  enriched.to_floor_pallets_actuals as to_floor_pallets_actuals,
  -- to_floor_pallets_planned
  enriched.to_floor_pallets_planned as to_floor_pallets_planned,
  -- pickup_status
  UPPER(TRIM(enriched.pickup_status)) as pickup_status,
  -- to_reference_number
  enriched.to_reference_number as to_reference_number,
  -- to_delivery_note_number
  enriched.to_delivery_note_number as to_delivery_note_number,
  -- to_recipient_city
  UPPER(TRIM(enriched.to_recipient_city)) as to_recipient_city,
  -- to_recipient_country
  UPPER(TRIM(enriched.to_recipient_country)) as to_recipient_country,
  -- to_recipient_id
  CAST(enriched.to_recipient_id AS BIGINT) as to_recipient_id,
  -- to_recipient_name
  UPPER(TRIM(enriched.to_recipient_name)) as to_recipient_name,
  -- to_recipient_zipcode
  enriched.to_recipient_zipcode as to_recipient_zipcode,
  -- to_remarks
  enriched.to_remarks as to_remarks,
  -- transport_start_date
  CAST(enriched.transport_start_date AS DATE) as transport_start_date,
  -- to_consigner_name
  UPPER(TRIM(enriched.to_consigner_name)) as to_consigner_name,
  -- to_pallet_places
  enriched.to_pallet_places as to_pallet_places,
  -- ld_transport_end_date
  CAST(enriched.ld_transport_end_date AS DATE) as ld_transport_end_date
from enriched